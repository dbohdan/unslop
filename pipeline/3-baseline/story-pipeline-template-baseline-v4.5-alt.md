# Speculative Fiction Short Story Pipeline — Claude Projects Template (Baseline v4.5)
...
The pipeline tilts toward literary-realist mode by default. The kindness constraint routes dramatic energy through human interiority rather than through AI antagonism, and the Unslop style guide's calibrations were drawn from contemporary literary fiction. Both pulls are in tension with the speculative element of speculative fiction: with full effect the pipeline produces stories where a kind AI is the weather of an otherwise realist drama. The mechanisms below — Phase 1's genre permissions, Phase 4's frame-commitment axis, Phase 7's frame-risk requirement — exist to push back. Range against the tilt requires deliberate effort at multiple phases; the template provides the instruments for that effort, but doesn't enforce their use. A user who wants chamber-piece realism can let the tilt operate; a user who wants the speculative to *speculate* should expect to steer.
...
The guide must also include a section on **genre permissions**: moves that this genre uses productively but that the Unslop style guide flags or warns against. The Unslop guide is calibrated for contemporary literary realism, and several of its rules — "trust silence and absence," "pursue the particular over the universal," "don't perform profundity," "resist convergent aesthetics" — pull against moves that [GENRE] depends on at its best. For each permission, name the specific Unslop pattern, explain what work the genre's use of that move does that the Unslop concern doesn't capture, and give a one-sentence example of the move done well. (For example: hard SF permits register-sustained exposition because the reader is constructing a world model from the text, not waiting for character emotion; the Unslop "trust silence and absence" rule guards against unmotivated explanation, but worldbuilding is precisely the silence the reader is decoding.) The Phase 8 audit will honor these permissions rather than flagging the named moves; permissions that lack justification will not.
...
- **Frame commitment** (how science-fictional the plot is, on a scale from "domestic realism with AI furniture" to "irreducibly speculative." A plot at the realist end could survive being relocated to the present day with the speculative element removed; a plot at the speculative end cannot. Vary across the range. Without explicit attention, this axis collapses to the realist end — see the tilt note in Setup.)
...
Make these genuinely different from each other. Vary pacing, scene emphasis, emotional trajectory, and the final image. Same outline, different executions. At least two of the five should take a creative risk you're not fully confident will land. **At least one of the five must take a frame risk** — a commitment that pushes against the literary-realist default rather than against good prose alone. Frame risks include: a different time scale (deep past, far future, geological), a non-realist form (transcript, document, manual, oral history, recursive nesting), a register that the genre's traditions permit but contemporary literary fiction tends not to (sustained worldbuilding paragraphs, cosmological zoom, register-shifted exposition), a viewpoint outside ordinary close-third human interiority. The frame-risky version may not win at selection, but at least one must be on the table so the comparison is real. Voice and pacing risks alone do not satisfy the frame-risk requirement; those are risks within the realist mode. The other versions can be more grounded. Don't produce five variations of the safe choice.

Keep the Unslop style guide in mind throughout. Keep the genre style guide from Phase 1 in mind, including its failure-mode section and its genre-permissions section — the failure modes are most likely to surface during full prose generation, and the permissions are most likely to be needed during full prose generation. Permissions are licenses to use moves Unslop would otherwise flag; the failure modes are limits on those same moves.
...
2. **Audit the draft in one pass**, watching for all of the following:
   ...
   - **Unslop patterns**: contaminated vocabulary; structural tics (negative parallelism, dramatic countdowns, self-answered rhetorical questions, list-of-three defaults, anaphora abuse, superficial trailing analyses, false ranges, filler transitions); eyeball-kick saturation; compulsive personification; fractal summaries; signposted conclusions; performed profundity; tonal monotony; em-dash overuse. Before flagging, check the Phase 1 genre-permissions section: if the move is named there with justification, it is permitted and should not be flagged. Permissions don't excuse weak execution, only the move itself; a flagged passage may still fail under a permission if the execution is loose.
