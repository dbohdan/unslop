# New Wave SF Short Story Pipeline — Claude Projects Template (Baseline v3.1)
...
**Why ten outlines instead of twenty in Phase 5.** Phase 3 (plots) and Phase 4 (structures) benefit from 20 candidates because the space of possible plots and structures for a given conflict is genuinely wide — a larger pool materially increases the chance of finding something that breaks the default. Phase 5 (outlines, already constrained by the winning plot and structure) occupies a narrower space; 20 outlines cluster heavily on shared scene counts, beat distributions, and final images regardless of the diversity instruction. Ten candidates give the funnel real choices without burning generation effort on near-duplicates. Phases 3 and 4 stay at 20.
