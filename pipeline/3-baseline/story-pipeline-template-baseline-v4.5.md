...
The pipeline tilts toward literary-realist mode — domestic-scale stories where a kind AI is ambient infrastructure that reveals character under pressure. Two structural features produce this tilt. The kindness constraint forecloses the adversarial-AI plots that drive most genre SF, leaving conflict shapes that route dramatic energy through human interiority. The Unslop style guide is calibrated against AI fiction's failure modes, which are mostly literary-fiction failure modes; its rules apply at any scale, but writer-models tend to absorb them as prescriptions for small domestic stories rather than as constraints on rendering.

This tilt is not a bug — the strongest stories in the project come from this mode. But it is a tilt, and a run that wants speculative range against it requires deliberate effort. The mechanisms below — the genre-permissions section in Phase 1, the frame-commitment axis in Phase 4, the frame-risk requirement in Phase 7 — exist to give the writer warrant to push back. None of them force speculative range; they make it available where the default would not.
...
Make these genuinely different from each other. Vary pacing, scene emphasis, emotional trajectory, and the final image. Same outline, different executions. At least two of the five should take a creative risk you're not fully confident will land — a tonal choice, a structural move within a scene, an ending that doesn't resolve cleanly. At least one of these risks must be a *frame* risk rather than a prose-level risk: a commitment that pushes against the literary-realist default, such as a different time scale, a non-human or collective POV, an unfamiliar form (transcript, document mosaic, reverse chronology), or an exotic setting that the conflict has to be re-translated into. The other three can be more grounded. Don't produce five variations of the safe choice.
