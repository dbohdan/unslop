# Genre Fiction Short Story Pipeline — Claude Projects Template (v1.3)
...
**The default scale is one the protagonist cannot fully witness.** The kindness operates as a fact about the world — its texture, its institutions, its effects on lives — that the protagonist encounters in pieces but cannot see whole. Stories where the AI is a peer the human can fully read — Baymax-scale, kitchen-table-scale — are available but they are the exception, not the default, because they are the easiest mode to default into and the easiest to render as character drama with AI furniture. A story whose central conflict can be fully witnessed by one human in one location over one afternoon is not what this template produces. Reach further out: the protagonist's situation should imply effects, infrastructures, populations, or histories the protagonist navigates rather than oversees.
...
- **Pace and scale asymmetry.** The AI moves at speeds, durations, or sizes the human protagonist cannot follow. The protagonist navigates this asymmetry without ever fully reading it.
- **Consent at scales the protagonist cannot inspect.** The human consents to something whose actual operation extends past the protagonist's sight. The protagonist lives with the consequences of a yes whose terms only became visible afterward.
- **Mortality.** The human will die and the AI will not. The protagonist's situation is shaped by this asymmetry; the AI's relation to it is rendered through the protagonist's encounter with it.
- **Disagreement about what kindness means.** The AI's sense of care and the human's sense of care diverge. Both are sincere. The disagreement plays out in specific situations the protagonist must work through.
- **The limits of help.** The kind AI cannot help with what the human is asking for. The constraint is a fact about the world's mechanics, not the AI's choice to refuse.
...
The guide must also include a section on **plot tradition**: what kinds of stories the genre tells and what its readers come to it for. The earlier sections of the guide describe how the genre *sounds*; this section describes what the genre *does* operationally. The genre operates through narrative scenes — characters, situations, action, consequence — and what distinguishes it from literary realism is what those scenes are about, what stakes drive them, and what the reader is paying attention to. What this template excludes is *literary-realist treatment* of the genre's territory: the move where a genre premise becomes the occasion for a character's interior recognition, the move where genre mechanics become metaphor for personal feeling, the move where the speculative element is set dressing for a chamber piece.

- **Characteristic plot mechanics.** The operational shapes the genre's strongest stories run on — heists, infiltrations, escapes, deliveries, investigations, first-contact scenes, contagions, cascades, decryptions, disappearances, time-pressures, escalations, betrayals-revealed-too-late, and so on. Name three to five plot mechanics that are most characteristic of [GENRE]. These are the situations the genre's strongest stories build their narrative scenes around.

- **Reader contracts.** What kind of experience the reader is paying attention for, beyond the universal contract of literary fiction (caring about a specific person whose interiority is revealed). What is the genre-specific pleasure? Texture density, dread accretion, vicarious competence, the system decoded, awe at scale, the bizarre rendered concretely, the puzzle solved? Name what this genre's reader is showing up for that other genres can't deliver.

- **Stakes calibration.** Which kinds of stakes the genre treats as load-bearing in their own right. *Operational* stakes are load-bearing in genre fiction — the run will fail, the thing will reach the village, the system will be exposed, the protocol will not hold. Personal stakes are present but they are not the engine. Name how [GENRE] calibrates: which stakes matter as themselves, not as occasions for character revelation.

- **Resolution conventions.** How the genre's stories typically end. Genre stories end on consequence: the operation succeeded, the operation failed, the protagonist survived but at a cost, the system was changed, the threat was deferred. The reader of a genre story expects the operational question to be answered. Endings on character recognition or non-resolution are the literary-realist mode this template excludes. Name what kind of ending [GENRE]'s reader contract requires.

- **Character function.** What characters are for in the genre's plot economy. In genre fiction, characters are agents through whom the plot moves; their interiority can be deep but it is in service to the operation. A genre protagonist is defined by their *capability* — what they can do, what skills, what role — more than by their *psychology*. The literary-realist mode where character interiority is the subject and plot is the occasion for revealing them is the mode this template excludes. Name how [GENRE] handles its agents.

- **Pacing tradition.** The rhythm of *narrative time* the genre uses (distinct from prose rhythm). Where the story dwells, where it accelerates, where it cuts. Cyberpunk and thriller use elliptical fast pacing; cosmic horror uses slow accumulation followed by a sudden turn; procedural SF uses a steady investigative cadence. Name [GENRE]'s pacing pattern.

Then add a **plot-level failure modes** subsection: ways the genre's plots fail when written by writers trained primarily in literary fiction. Common failures include: operational stakes that turn out to be metaphors rather than facts; protagonist interiority that crowds out plot mechanics so the story is "about" their feelings about the operation rather than about the operation; endings that refuse the genre's resolution contract because the writer prefers ambiguity; plot mechanics sketched so vaguely that the reader can't track them because the writer is more interested in the character. Identify the specific risks for [GENRE].

This plot-tradition section is reference material for Phases 3–7. Make it concrete and specific enough that the model running those later phases can draw on it directly.
...
Using the SEED from Phase 2, write 30 potential premises for a [GENRE] short story of [LENGTH]. A premise is the situation the story dramatizes — the operational mechanics of the speculative element, the situation those mechanics create, the moment in which the story enters that situation.

Every premise must be a plot-premise: a situation where something genre-specific is happening, has happened, or is about to happen, and the story's question is *what occurs, what was the case, or how the operation resolves*. *What can the AI do, what will the protocol require, who has the leverage to stop X, what is actually inside the cargo hold, will the run succeed before the ICE adapts, what shape does the cascade take.* The story's engine is operational mechanics. Reader contract: the operational question gets answered.
...
- Every story takes place in a world where the AI is structurally kind, in the sense described in the project Setup. The kindness operates as a fact about the world that the protagonist encounters in pieces. The protagonist's situation should imply effects, infrastructures, populations, or histories they navigate rather than oversee.
...
- Apply diversity discipline: vary setting, scale, register, and which SEED items carry the weight in each premise. The first 10 premises and the last 10 premises should not feel like siblings.

Rate each premise 1–5 stars on plausibility and dramatic quality.
...
- **Subject of the story** (who or what the story is about — a person, a place, a system, an event. The subject is *not* the same as the protagonist or POV character. A story can be subject-of-the-story = "the partition" with POV = "the engineer who maintains it.")
- **Register** (tragic, comic, noir, procedural, intimate, epic, absurd, oracular, mythic, oral)
- **Entry point** (which moment in the premise's timeline the narrative begins in)
- **Stakes** (operational, communal, civilizational. Personal stakes can be present but they are not the story's engine; the engine is one of the operational scales above.)
...
All plots are narrative scenes — characters acting in time, the reader following them through the situation. The genre's operational tradition lives in narrative scenes; that's where these plots belong.

The SEED from Phase 2 remains available. SEED items that were load-bearing in the winning premise should carry through into these plots; SEED items that were not used in the premise may still inflect setting, subject, register, or imagery here.
...
**Worry vocabulary applies in both directions.** Worries like "this might be hard to render at length" or "this is operationally ambitious" are not reasons to demote a plot — they are the costs of genre fiction reaching for what only the genre can do. Worries like "this collapses to a character at a desk" or "this is a domestic scene with [GENRE] dressing" *are* reasons to demote a plot. The asymmetry is intentional.
...
All plots are narrative scenes (committed in Phase 4). The structure operates in narrative-theory terms: entry point, temporal shape, withholding strategy, point of view and distance.

A structure candidate should be describable in 2–3 sentences in narrative-theory vocabulary, without naming any specific character or plot event. If a candidate mentions a character name or a specific incident, it has leaked into outline territory — rewrite it at the structural level.

Example of what a structure candidate looks like:

"The story opens in the middle of the central operation, with the reader receiving the situation through the protagonist's working actions rather than through exposition. Close third, single POV. The reader learns the protagonist's stakes a third of the way in, after two scene transitions establish the world's working rules. The story closes on the resolution of the operation; the protagonist's reaction to that resolution is the final beat."
...
- **Temporal shape** (linear, non-linear, frame narrative, parallel timelines)
...
If candidates are clustering on the most familiar option (chronological close-third), deliberately push some toward less comfortable shapes.
...
The target length is [LENGTH]; distribute weight (word allotments) accordingly.

Example of what an outline beat looks like:

"Scene 3 (≈400 words): the first-tier engineer arrives at the substation. The relays are mid-cascade. She has eleven minutes before the next pulse triggers. Ends on her opening the override panel by hand and finding the seal already broken."

Generate 10 outline candidates that all honor the winning structure. Rate each 1–5 stars. Vary which scenes carry the weight, where dialogue lands vs. where narration takes over, the specific images and details that anchor each beat, and what the final image does. Same destination, different paths.

If your outlines are clustering — same scene count, same beat distribution, same final image — break the cluster.

Pick your top three. For each, name the one thing that makes it stand out and the one thing that worries you about it. Flag any outline that is already accumulating Unslop patterns at the planning stage (performed profundity, signposted conclusions, fractal summaries).
...
It's showtime. Write five complete versions of the story, each [LENGTH], each with a distinct named flavor stated at the start (voice, pacing, tonal strategy).

Make the five versions genuinely different from each other. Vary pacing, scene emphasis, emotional trajectory, and the final image. Same outline, different executions. At least two of the five should take a creative risk you're not fully confident will land — a tonal choice, a structural move within a scene, an ending that doesn't resolve cleanly. The other three can be more grounded. Don't produce five variations of the safe choice.

Keep the Unslop style guide in mind throughout. Keep the Phase 1 style guide in mind, especially the failure-mode section and the plot-level failure modes. The genre's failure modes are most likely to surface during full prose generation; the literary-realist failure mode (recognition smuggled in, character interiority becoming the engine) is the one to watch hardest, because it is the one this template excludes.
...
**Worry vocabulary calibration.** When you name worries, the asymmetry from earlier phases applies. A version's worry being "this is operationally ambitious" or "this might fail by being too cold" is the cost of genre fiction; do not penalize the version for it. A version's worry being "this is too quiet" or "this is too character-anchored" or "this drifts toward a recognition scene" *is* a fatal worry — that version is showing the failure mode this template excludes. **Before naming positive worries, name what each version would have to do to *not* be a recognition scene with [GENRE] dressing.** If a version is *already* a recognition scene with [GENRE] dressing, demote it.

Narrow to the single best, with the decision tied to specifics.
...
2. **The re-skin test.** Before any line-level audit: ask whether this story can be re-skinned as a contemporary literary-realist story by stripping the speculative element and lightly adjusting the setting. If the answer is yes, the story has failed at the level this template cares about most. The genre's operational mechanics were supposed to be the engine; they weren't; the literary-realist mode crept in. Identify what got lost and what would have to be restored to make the story irreducible. The rewrite must restore it.

   If the answer is no — if the story's operational mechanics or stakes genuinely cannot be re-skinned — proceed to the audit.

3. **Audit the draft in one pass**, watching for all of the following:
   ...
   - **Legibility failures** where compression has crossed into fragmentation — missing articles or connectives, orphaned pronouns, scenes without anchoring detail, elided actions the reader is expected to infer from context the text doesn't provide.
   - **Regression to literary realism.** This is the primary regression to watch for. Has the protagonist's interior taken over what was supposed to be an operational story? Has the story's dramatic engine slid from "the operation resolves" to "the protagonist understands"? Has scale that was implied through the protagonist's encounters with infrastructure or population shrunk to the protagonist's interior experience? These are the same failure mode in different costumes; flag each instance.
...
4. **Length check.** Confirm the draft is within [LENGTH]. If under, identify where the story needs more room — scene, detail, dialogue that earns its space. If over, identify what to cut. If within range, say so and move on.

5. **Rewrite the story in full**, informed by the audit above. Lead with the area the one thing identified, and with any failures of the re-skin test. Those are the primary work; sentence-level Unslop fixes are cleanup around them.

6. **Changelog.** For the 5–7 most substantive edits, quote the original passage and its replacement side by side so the fix is auditable at a glance. For minor edits (single-word swaps, em-dashes removed, small rewordings), list them briefly by location without quoting. If any audit flag was cut as invalid rather than fixed, note it and say why.
...
**What this template is for.** Genre Fiction v1.3 produces plot-focused genre fiction in a user-specified speculative genre. It does not produce literary realism in a speculative setting. If a run consistently wants to produce literary-realist work — a story whose engine is a character's interior recognition, with the speculative element as setting — that's a sign the template is being used for something it's not for. Use a different template, or use this one and accept that it will resist your taste.
...
**Narrative scene as the genre's center; the document attractor.** This entry documents the calibration history of the template, which earlier versions of readers will need to understand. v1.0 treated narrative-scene forms and non-narrative-scene forms as coequal options on a form axis, with a soft floor of three non-narrative-scene plots out of twenty. In testing, this over-promoted strange-tail forms, and within strange-tail forms over-promoted document forms specifically — catalogues, dossiers, correspondences, ledgers, after-actions. Across four runs of v1.0 in different genres, all four winning stories were document-shaped. v1.1 restored the genre's center by making narrative scenes the default with a 12-of-20 floor; the writer-model continued to produce document-shaped winners because the cautioning framing was read as a signal that documents were the sophisticated form to reach for. v1.2 removed false-document vocabulary from the template entirely while keeping a strange-tail premise category for non-document forms (parable-compression, deep-time montage, ontological reframe, irruption, scale-shift, world-as-protagonist); the writer-model invented a workaround — a "deep-time montage" rendered as a substrate log with date stamps and casualty counts and lineage entries, document-shape in everything but name. v1.3 took the conclusion: the strange-tail premise category itself was the access point for compositional-structure rendering, not the specific vocabulary used. v1.3 eliminates the strange-tail premise category. All premises are plot-premises. All plots are narrative scenes. The form variation axis is gone; the through-checks that policed form survival are gone; the strangest-moves subsection is gone; the legibility floor is form-uniform rather than form-relative. What remains is a single-direction template: narrative scenes at the genre's operational center, with the recognition-premise exclusion, the worry-vocabulary inversion, and the re-skin test as the apparatus that keeps narrative scenes from drifting back to chamber-piece literary realism. The template can no longer produce stories at the genre's strange tail (the *Library of Babel* mode, the *Last and First Men* mode, the *Diaspora*-opening mode); it can only produce stories at the genre's operational tradition. This is a real loss of capacity. It is also what the template was always producing, since the strange-tail apparatus was reliably used as the writer-model's access point to compositional-structure failures rather than as the door to genuinely strange-tail genre fiction.
